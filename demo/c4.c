...
int *e, *le,  // current position in emitted code
    ...
    *brk_i,     // break statement patch-up pointer
...
void stmt()
{
  ...
  case Switch:
    ...
    if (tk == ')') next(); else { printf("%d: close paren expected\n", line); exit(-1); }
    a = cas; *++e = JMP; cas = ++e;
    b = brk_i; d = def; brk_i = def = 0;
    stmt();
    *cas = def ? (int)def : (int)(e + 1); cas = a; 
    while (brk_i) { a = (int *)*brk_i; *brk_i = (int)(e + 1); brk_i = a; printf("break_patch: %d\n", brk_i); }
    brk_i = b; def = d;
    return;
  case Case:
...
int main(int argc, char **argv)
{
  ...
  while (tk) {
    ...
    while (tk != ';' && tk != '}') {
      ...
      printf("Class: %d\n", id->class);
      if (id->class) { printf("%d: duplicate global definition: %d\n", line, id->class); return -1; }
      next();
      id->type = ty;
      if (tk == '(') { // function
